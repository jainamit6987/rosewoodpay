...
// Reached by tapping the society name in AdminHomeScreen's own header - the
// same "tap the identifying header element to reach its own profile/detail
// screen" pattern ResidentHomeScreen's circular house-number avatar already
// established for HouseProfileScreen. This is where the read-only society
// facts (UPI VPA, UPI payee name, timezone, registered date) that used to
// sit in a card directly on SocietyScreen now live, once that screen became
// Reports/Actions only.
//
// Backed by the same GET /society (routes/society.js) SocietyScreen itself
// still uses for its own per-society Reports/Actions - not a new
// single-society endpoint - so this renders a list for the same reason
// that screen does (a future multi-society Admin/Committee member is
// handled for free), even though in practice it is always exactly one
// society today.
export default function SocietyProfileScreen({ onBack }) {
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text style={styles.title}>Society Profile</Text>
      </View>
